  ...
  if (!isAuthenticated) {
    return (
      <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-sky-50 via-white to-amber-50">
        ...
        <motion.div aria-hidden="true" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pointer-events-none absolute -left-24 top-10 h-72 w-72 rounded-full bg-sky-300/30 blur-3xl" />
        <motion.div aria-hidden="true" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="pointer-events-none absolute -right-20 bottom-12 h-72 w-72 rounded-full bg-amber-300/30 blur-3xl" />
        ...
        <div className="relative z-10 mx-auto grid min-h-screen w-full max-w-6xl items-center gap-8 px-4 py-12 lg:grid-cols-2 lg:px-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.45 }} className="hidden rounded-3xl border border-slate-200 bg-white/80 p-8 shadow-[0_22px_60px_-28px_rgba(15,23,42,0.4)] backdrop-blur lg:block">
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-sky-700">
              <ShieldCheck className="h-3.5 w-3.5" /> {i18next.t('auto.secure_admin_area_1li5cqy')}
            </div>
            <h1 className="mt-4 text-4xl font-black leading-tight tracking-tight text-slate-900">{i18next.t('auto.notice_control_panel_1i4dp1q')}</h1>
            <p className="mt-3 max-w-md text-slate-600">{i18next.t('auto.manage_notices_notifications_tenders_and_upcoming_events_1kukv9v')}</p>
            <div className="mt-8 space-y-3">
              <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm">{i18next.t('auto.upload_file_attachments_and_publish_with_schedule_19ock66')}</div>
              <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm">{i18next.t('auto.pin_critical_notices_and_control_visibility_instantly_vznpmi')}</div>
              <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 shadow-sm">{i18next.t('auto.designed_for_fast_updates_during_admissions_and_121yi3l')}</div>
            </div>
          </motion.div>
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/90 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.45)] backdrop-blur">
              <div className="border-b border-slate-200 bg-gradient-to-r from-sky-50 via-white to-amber-50 px-6 py-5">
                <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-slate-600">
                  <Sparkles className="h-3.5 w-3.5 text-sky-600" /> {i18next.t('auto.admin_sign_in_6swgf2')}
                </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-sky-50/40">
      ...
      <div className="border-b border-slate-200 bg-white/80 backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 px-6 py-6 text-white shadow-[0_24px_60px_-30px_rgba(15,23,42,0.7)]">
            <motion.div aria-hidden="true" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pointer-events-none absolute -right-12 -top-10 h-40 w-40 rounded-full bg-sky-300/20 blur-3xl" />
            <motion.div aria-hidden="true" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }} className="pointer-events-none absolute -bottom-10 left-16 h-36 w-36 rounded-full bg-amber-300/20 blur-3xl" />
            ...
            <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.98 }} onClick={() => { setSuccessMessage(''); setShowForm(true); setEditingNotification(null); }} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-900 shadow transition hover:bg-slate-100">
                  <Plus className="h-4 w-4" /> {i18next.t('auto.add_notice_1mv4k0u')}
                </motion.button>
                <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.98 }} onClick={handleLogout} className="inline-flex items-center gap-2 rounded-xl border border-white/35 bg-white/10 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-white/20">
                  <LogOut className="h-4 w-4" /> {i18next.t('auto.logout_1c182un')}
                </motion.button>
              </div>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        {showForm ? (
          <AdminNotificationForm notification={editingNotification} onSave={handleSaveNotification} onCancel={() => { setShowForm(false); setEditingNotification(null); }} />
        ) : (
          <div className="py-2">
            <div className="mb-4 flex items-center gap-2">
              <button type="button" onClick={() => setActiveTab('notices')} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === 'notices' ? 'bg-sky-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}>
                Notices
              </button>
              <button type="button" onClick={() => setActiveTab('events')} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === 'events' ? 'bg-sky-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}>
                Admin Events
              </button>
            </div>
            ...
            {activeTab === 'notices' ? (
              <>
                {successMessage && <div className="mb-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">{successMessage}</div>}
                <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_20px_50px_-35px_rgba(15,23,42,0.45)]">
                  {loading ? (
                    <div className="px-6 py-10 text-center text-slate-500">{i18next.t('auto.loading_1ooa6x9')}</div>
                  ) : notifications.length === 0 ? (
                    <div className="px-6 py-10 text-center text-slate-500">{i18next.t('auto.no_notices_found_xpxstx')}</div>
                  ) : (
                    <div className="overflow-x-auto overscroll-x-contain">
                      <ul className="grid min-w-[860px] gap-4 p-4 md:p-6">
                        {notifications.map((notification, index) => (
                          <motion.li key={notification.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center flex-wrap gap-2">
                                  <p className="text-lg font-semibold text-slate-900 truncate">{notification.title}</p>
                                  <span className="inline-flex items-center rounded-full bg-sky-100 px-2.5 py-1 text-xs font-semibold text-sky-800">{publishLabelMap[notification.publish_to] || notification.publish_to}</span>
                                  {notification.pinned && <span className="inline-flex items-center rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-800">{i18next.t('auto.pinned_1jx2yl9')}</span>}
                                  {!notification.published && <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{i18next.t('auto.draft_3gnfc0')}</span>}
                                </div>
                              ...
                              <div className="flex shrink-0 flex-col gap-2 lg:min-w-[220px] lg:items-end">
                                ...
                                <div className="flex w-full flex-wrap gap-2 lg:justify-end">
                                  <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.95 }} onClick={() => handleEditNotification(notification)} className="inline-flex items-center gap-2 rounded-xl border border-sky-200 bg-sky-50 px-3.5 py-2 text-sm font-semibold text-sky-700 transition hover:border-sky-300 hover:bg-sky-100 hover:text-sky-900" title={i18next.t('auto.edit_yjfqt5')}>
                                    <Edit className="h-4 w-4" /> Edit
                                  </motion.button>
                                  <motion.button whileHover={{ y: -1 }} whileTap={{ scale: 0.95 }} onClick={() => handleDeleteNotification(notification.id)} className="inline-flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-3.5 py-2 text-sm font-semibold text-rose-700 transition hover:border-rose-300 hover:bg-rose-100 hover:text-rose-900" title={i18next.t('auto.delete_16wmut8')}>
